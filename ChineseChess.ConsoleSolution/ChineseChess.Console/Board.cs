    ...
    public class Board
    {
        ...
        public bool IsInCheck(Player player)
        {
            ...
            foreach (var kvp in opponent.Pieces)
            {
                ...
                if (CanMove(opRow, opCol, genRow, genCol, piece))
                {
                    return true;
                }
            }
        ...
        private bool CanMove(int fromRow, int fromCol, int toRow, int toCol, Piece piece)
        {
            ...
            foreach (var fromKvp in player.Pieces)
            {
                ...
                for (int toR = 0; toR < Rows; toR++)
                {
                    for (int toC = 0; toC < Columns; toC++)
                    {
                        ...
                        if (CanMove(fromPos.row, fromPos.col, toR, toC, piece))
                        {
                            // Simulate move
